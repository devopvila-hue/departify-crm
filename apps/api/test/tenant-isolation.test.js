...
async function createOrg(label) {
    ...
    if (!login.ok)
        throw new Error(`login failed for ${label}: ${login.status} ${await login.text()}`);
...
describe('cross-tenant isolation', () => {
    ...
    it('list endpoints only return org A resources', async () => {
        const res = await jsonOrThrow(await fetch(`${BASE_URL}/api/v1/contacts?pageSize=100`, authed(a.cookie)));
        const items = res.items;
        for (const item of items)
            expect(item.id).not.toBe(b.contactId);
        const deals = await jsonOrThrow(await fetch(`${BASE_URL}/api/v1/deals?pageSize=100`, authed(a.cookie)));
        for (const d of deals.items)
            expect(d.id).not.toBe(b.dealId);
    });
    it('service API key isolation — a key bound to org A cannot read org B resources', async () => {
        // Create a key for org A
        const create = await jsonOrThrow(await fetch(`${BASE_URL}/api/v1/service-keys`, {
            method: 'POST',
            headers: { 'content-type': 'application/json', cookie: a.cookie },
            body: JSON.stringify({ name: 'test-key', scopes: [] }),
        }));
        const { secret, prefix } = create;
        expect(prefix.startsWith('dep_ak_live_')).toBe(true);
        // Use the key to call /auth/me
        const me = await jsonOrThrow(await fetch(`${BASE_URL}/api/v1/auth/me`, {
            headers: { authorization: `Bearer ${secret}` },
        }));
        expect(me.organizationId).toBe(a.orgId);
    ...
    it('IDOR on /attention: org A only sees its own counts', async () => {
        const data = await jsonOrThrow(await fetch(`${BASE_URL}/api/v1/attention`, authed(a.cookie)));
        ...
        expect(dataB.summary.contactsTotal).toBe(1);
    });
});
void sha256;
void randomToken;
