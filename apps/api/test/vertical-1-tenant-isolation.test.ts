...
const DATABASE_URL = process.env.DATABASE_URL ?? 'postgres://postgres:***@localhost:5432/departify_crm_test';
  ...
  if (fs.existsSync(migrationsDir)) {
    ...
    for (const f of files) {
      ...
      for (const stmt of statements) {
        try { await adminDb.execute(sql.raw(stmt)); } catch { /* ignore IF NOT EXISTS races */ }
      }
    }
  }
